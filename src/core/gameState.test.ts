...
describe('gameState', () => {
  it('starts the player at the room center, at rest', () => {
    ...
    expect(s.player.pos).toEqual({ x: CX, y: CY });
  ...
  it('moves the player in open space at the expected speed', () => {
    ...
    expect(s.player.pos.x).toBeCloseTo(CX + PLAYER_SPEED * FIXED_DT, 10);
    expect(s.player.pos.y).toBeCloseTo(CY, 10);
  });

  it('normalizes diagonal movement (not faster than one axis)', () => {
    const s = createGame(1);
    tick(s, { moveX: 1, moveY: 1 }, FIXED_DT);
    const dx = s.player.pos.x - CX;
    const dy = s.player.pos.y - CY;
